...
| README inaccuracy | `README.md:128` claims `firebase_options.dart`/`google-services.json` are gitignored — they are not (only `lib/config/api_config.dart` is) | Fix the claim; decide deliberately whether these files *should* be gitignored per-environment once the dev/staging/prod split lands (Phase 2), since each environment will need its own. |
