...
#### FOLLOW-UP — 2026-09-10, fresh-clone build correctness

The 2026-09-09 remediation left `lib/config/api_config.dart` gitignored and untracked, which meant a genuinely fresh clone (no local dev history) did not contain the file at all — `ai_service.dart` imports it directly, so this broke `flutter analyze` (3 real compile-time errors: `uri_does_not_exist`, `const_initialized_with_non_constant_value`, `undefined_identifier`) and `flutter build` (hard compile failure) on any clean checkout, confirmed empirically via a fresh-checkout simulation. The fix is **not** to have CI (or anything else) manufacture the file at build time — that would just be a different, still-fragile secret-injection-shaped mechanism. Instead:
- `lib/config/api_config.dart` is now **tracked**, containing only `static const String groqApiKey = '';` — no real credential, ever. `.gitignore`'s exclusion for this path is removed; the file's own header comment now carries the "do not put a real key here" warning that the removed `.gitignore` comment used to carry.
- `lib/config/api_config.dart.example` is **removed** — with the real tracked file now itself the safe, empty, canonical version, keeping a separate template file would be a second, confusing source of truth for the same content.
- The Dockerfile's `ARG API_CONFIG_DART` / base64-decode step is **removed**, not just deprecated — it's no longer needed (the tracked file is already present via `COPY . /app`) and was actively harmful to keep: with the ARG unset, `echo "$API_CONFIG_DART" | base64 -d` decodes to an empty string, silently overwriting the good tracked file with a zero-byte file and reintroducing the exact same compile failure. Any future re-introduction of a per-environment build-time AI config for this Dockerfile would be a step backward from the target architecture (client-side proxy elimination) and should not happen — the Dockerfile no longer has any mechanism to accept an AI provider secret at all.
- This does not change the finding above or its resolution status — H-1 remains resolved; this is a correctness follow-up ensuring the resolved state is also fresh-clone-buildable.
...
- **Location**: `android/app/google-services.json`, `lib/firebase_options.dart` — both git-tracked (the README's claim that these are gitignored is inaccurate). **Updated 2026-09-10**: this used to note `lib/config/api_config.dart` as the one file that genuinely was gitignored — that's no longer true either, since it's now a tracked, safe, empty placeholder (see H-1's 2026-09-10 follow-up); the README's gitignore claim is therefore even further from current reality than when originally written, which strengthens rather than changes the recommendation below.
